...
# Calculate Pressure Drops for PVC of Ideal Roughness
e_pvc = 0.0015/1000
f_pvc = np.array([friction_factor(i, e_pvc, model='cole') for i in Res])
vel_pvc = np.sqrt(dP/(0.5*f_pvc*rho*(l/d)))
dev = abs(vel - vel_pvc)/vel
...
# Titles
fig.suptitle('Velocity for Experiment vs. Ideal PVC', fontsize=11)
...
axs.plot(dP, vel, linewidth=1.0,
               color='black',
               label='Experiment')
axs.plot(dP, vel_pvc, linewidth=0.75,
               color='black',
               label='PVC', linestyle='dashed')
axst.plot(dP, dev, linewidth=1.0, color='red', label='Deviation')



# Styling
axs.set_xlabel('Pressure Drop (Pa)')
axs.set_ylabel('Velocity '+r'$\frac{m}{s}$')
axst.set_ylabel('Deviation (%)')
