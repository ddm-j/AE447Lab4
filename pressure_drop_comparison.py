...
dP_pvc = 0.5*f_pvc*rho*(l/d)*vel**2
dev = (dP - dP_pvc)/dP
...
# Titles
fig.suptitle('Pressure Drop for Experiment vs. Ideal PVC', fontsize=11)
...
axs.plot(Res, dP, linewidth=1.0,
               color='black',
               label='Experiment')
axs.plot(Res, dP_pvc, linewidth=0.75,
               color='black',
               label='PVC', linestyle='dashed')
axst.plot(Res, dev, linewidth=1.0, color='red', label='Deviation')



# Styling
axs.set_xlabel('Re')
axs.set_ylabel('Pressure Drop (Pa)')
